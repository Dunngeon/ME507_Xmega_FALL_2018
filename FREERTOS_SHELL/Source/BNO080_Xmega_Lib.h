...
/*
  ...
  Adapted for Xmega - Ryan Dunn - 11/28/18
...
//specialized includes for this library
#include "twi.h"							//i2c functions
#include "math.h"							//used for pow() in qtoFloat
#include <util/delay.h>						//used for _delay_ms(), need to replace with FRTOS timers

//general includes for FRTOS and XMEGA
...
//There is no setup needed before running this library

//Platform specific configurations

//Define the size of the I2C buffer based on the platform the user has
//-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
#define I2C_BUFFER_LENGTH 25 //this is 25 bytes of buffer, can be whatever size it needs to be on Xmega.
...
/** class BNO080
 *  brief a Hillcrest Labs BNO080 device driver library for ATxmega.
 *  details I2C is accomplished using twi.h and twi.c
 */
class BNO080 {
  public:
	BNO080(emstream*, bool printDebug);		//constuctor class
    bool begin(uint8_t deviceAddress = BNO080_DEFAULT_ADDRESS); //By default use the default I2C addres, and use Wire port
	
	//debug printing disabled because needs type conversion to ostream, not important to operation.
    ...
    //Global Variables
	#define SHTP_HEADER_SIZE 4 //const
	uint8_t shtpHeader[SHTP_HEADER_SIZE]; //Each packet has a header of 4 bytes
  ...
  private:
    ...
    emstream *_debugPort; //The stream to send debug messages to if enabled. Usually Serial.
    ...
    int16_t rotationVector_Q1; //set in constructor
